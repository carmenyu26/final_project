...
WITH reaction_coverage AS (
    SELECT 
        ...
        SUM(CASE WHEN r.reaction_name IN ('R_FBP','R_FBA','R_F6PA','R_F6PP','R_F6Pt6_2pp') THEN 0 ELSE 1 END) * 20 AS cbb_cycle_cov,
    ...
    FROM organism o
    JOIN reaction r ON r.organism_id = o.organism_id
...
SELECT
    o.organism_name,
    SUM(CASE WHEN r.reaction_letter_id IN ('R_ACKr', 'R_ENO', 'R_FBP', 'R_PFOR') THEN 1 ELSE 0 END) AS oxygen_labile_count,
    SUM(CASE WHEN g.gene_name IN ('sodA', 'katE', 'katG', 'ahpC', 'ahpF') THEN 1 ELSE 0 END) AS oxygen_detox_count
FROM organism o
JOIN reaction r ON o.organism_id = r.organism_id
...
SELECT 
    o.organism_name,
    (COUNT(DISTINCT r.reaction_id) * 100.0 / total_krebs.krebs_reaction_count) AS krebs_coverage_percent
FROM organism AS o
JOIN reaction AS r ON o.organism_id = r.organism_id
...
CROSS JOIN (
    ...
    FROM pathway AS p2
    JOIN pathway_rxn_join AS pr2 ON p2.pathway_id = pr2.pathway_id
    JOIN reaction AS r2 ON r2.reaction_id = pr2.reaction_id
...
WITH total_organism_count AS (
  SELECT COUNT(DISTINCT r.organism_id) AS uc_organism_count
  FROM pathway p
...
SELECT 
  ...
  ROUND(
    (COUNT(DISTINCT organism_id) * 100.0 / toc.uc_organism_count),
    2
  ) AS coverage_percent
